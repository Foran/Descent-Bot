  ...
  if ((*file).is_open()) {
    ...
    if (!(*file).eof() && (*file).read(signature, 3)) {
      if (signature[0] == 'D' && signature[1] == 'H' && signature[2] == 'F') {
        dynamic_cast<CLog*>(mContext->getComponent("Log"))->Write(LogType::LogType_Debug, 100,
        ...
        while (!(*file).eof() && (*file).read(file_name, 13)) {
          ...
          if (!(*file).eof() &&
            ...
            dynamic_cast<CLog*>(mContext->getComponent("Log"))->Write(LogType::LogType_Debug, 60, "Found file: " + name +
                                                         " in hog");
...
fstreamptr CHog::get_Stream() const {
  ...
  dynamic_cast<CLog*>(mContext->getComponent("Log"))->Write(LogType::LogType_Debug, 200, "Attempting to open Hog (" +
                                                mFilename + ") Stream..." +
                                                ((*file).is_open() ?
                                                        "Success" :
                                                        "Failure"));
